...
*/
void setPixel(int row, int col, const u16 color) {
...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  ...
  for(int i = 0; i < height; i++) {
    DMA[MODE3].dst = videoBuffer + OFFSET((row + i), col, WIDTH);
    DMA[MODE3].cnt = width | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT | DMA_ON;
  }
}
...
void drawFullScreenImageDMA(const u16 *image) {
  ...
  DMA[MODE3].cnt = WIDTH * HEIGHT | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT | DMA_ON;
}
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  for(int i = 0; i < height; i++) {
    DMA[MODE3].src = image + (width * i);
    DMA[MODE3].dst = videoBuffer + OFFSET((row + i), col, WIDTH);
    DMA[MODE3].cnt = width | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT | DMA_ON;
  }
}
...
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) {
  for(int i = 0; i < height; i++) {
    DMA[MODE3].src = image + OFFSET(row + i, col, WIDTH);
    DMA[MODE3].dst = videoBuffer + OFFSET(row + i, col, WIDTH);
    DMA[MODE3].cnt = width | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT | DMA_ON;
  }
}
...
void fillScreenDMA(volatile u16 color) {
  ...
  DMA[MODE3].src = &color;
  DMA[MODE3].dst = videoBuffer;
  DMA[MODE3].cnt = WIDTH * HEIGHT | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT | DMA_ON;
}
